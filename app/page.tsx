...
export default function Home() {
  ...
  useEffect(() => {
    const onScroll = () => {
      ...
      if (sy > 80 && !doorsOpen) {
        setDoorsOpen(true)
        setTimeout(() => setHeroText(false), 200)
      }
    }
  ...
  async function generate(proj?: string) {
    ...
    try {
      const res = await fetch('/api/brief', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ project: p }) })
    ...
    <main style={{ background: N, color: C, fontFamily: 'Inter,sans-serif', minHeight: '100vh', overflowX: 'hidden' }}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        @keyframes glow{0%,100%{box-shadow:0 0 20px rgba(201,168,76,0)}50%{box-shadow:0 0 40px rgba(201,168,76,.15)}}
        ...
        .why-card{background:#0C1628;padding:36px;transition:background .3s,transform .3s;position:relative;overflow:hidden}
        ...
        .hero-search-input{flex:1;padding:20px 28px;background:transparent;border:none;color:#F0EAD6;font-size:15px;font-family:Inter,sans-serif}
        .hero-gen-btn{padding:20px 40px;background:#C9A84C;color:#060D1B;border:none;font-size:11px;letter-spacing:.12em;text-transform:uppercase;font-weight:600;cursor:pointer;min-width:180px;transition:background .2s}
        .hero-gen-btn:hover{background:#E2C473}
        .hero-gen-btn:disabled{opacity:.6;cursor:not-allowed}
        ...
        @media(max-width:768px){
          ...
          .hero-gen-btn{min-width:unset;width:100%}
        }
      `}</style>
      ...
      {/* ═══════════════════════════════════════ */}
      {/* HERO — BLUEPRINT + DOOR ANIMATION      */}
      {/* ═══════════════════════════════════════ */}
      <section ref={heroRef} style={{ height:'100vh', position:'relative', overflow:'hidden' }}>

        {/* BASE BLUEPRINT — assembles on load */}
        <div style={{
          position:'absolute', inset:0, zIndex:1,
          backgroundImage:`url('/blueprint.jpg')`,
          backgroundSize:'cover', backgroundPosition:'center',
          transform: assembled ? 'scale(1)' : 'scale(1.12)',
          filter: assembled ? 'blur(0px)' : 'blur(6px)',
          transition:'transform 1.6s cubic-bezier(0.34,1.2,0.64,1), filter 1.4s ease',
        }} />

        {/* FRAGMENT: Palm Jebel Ali — flies in from bottom-left */}
        <div style={{
          position:'absolute', bottom:0, left:0, width:'42%', height:'52%', zIndex:2, pointerEvents:'none',
          backgroundImage:`url('/blueprint.jpg')`, backgroundSize:'cover', backgroundPosition:'left bottom',
          clipPath:'polygon(0 20%, 100% 0%, 100% 100%, 0% 100%)',
          transform: assembled ? 'translate(0,0) scale(1)' : 'translate(-80px, 100px) scale(0.85)',
          opacity: assembled ? 1 : 0,
          transition:'transform 1.4s cubic-bezier(0.34,1.3,0.64,1) 0.2s, opacity 1s ease 0.2s',
          mixBlendMode:'luminosity',
        }} />

        {/* FRAGMENT: Dubai Islands — flies in from top */}
        <div style={{
          position:'absolute', top:0, right:'15%', width:'38%', height:'42%', zIndex:2, pointerEvents:'none',
          backgroundImage:`url('/blueprint.jpg')`, backgroundSize:'cover', backgroundPosition:'center top',
          clipPath:'polygon(10% 0%, 100% 0%, 100% 80%, 0% 100%)',
          transform: assembled ? 'translate(0,0) scale(1)' : 'translate(60px, -90px) scale(0.85)',
          opacity: assembled ? 1 : 0,
          transition:'transform 1.4s cubic-bezier(0.34,1.3,0.64,1) 0.35s, opacity 1s ease 0.35s',
          mixBlendMode:'luminosity',
        }} />

        {/* FRAGMENT: Palm Jumeirah — flies in from center-left */}
        <div style={{
          position:'absolute', top:'25%', left:'15%', width:'45%', height:'55%', zIndex:2, pointerEvents:'none',
          backgroundImage:`url('/blueprint.jpg')`, backgroundSize:'cover', backgroundPosition:'center',
          clipPath:'polygon(0% 10%, 85% 0%, 100% 90%, 15% 100%)',
          transform: assembled ? 'translate(0,0) scale(1)' : 'translate(-50px, 40px) scale(0.88)',
          opacity: assembled ? 1 : 0,
          transition:'transform 1.6s cubic-bezier(0.34,1.2,0.64,1) 0.1s, opacity 1.2s ease 0.1s',
          mixBlendMode:'luminosity',
        }} />

        {/* OVERLAY GRADIENT */}
        <div style={{ position:'absolute', inset:0, zIndex:3, background:'linear-gradient(180deg,rgba(6,13,27,.15) 0%,rgba(6,13,27,.05) 35%,rgba(6,13,27,.75) 100%)' }} />

        {/* SCANLINE */}
        <div style={{ position:'absolute', inset:0, zIndex:4, overflow:'hidden', pointerEvents:'none', opacity:.05 }}>
        ...
        {/* LEFT DOOR */}
        <div style={{
          position:'absolute', top:0, left:0, bottom:0, width:'50%', zIndex:10, overflow:'hidden',
          transform: doorsOpen ? 'translateX(-101%)' : 'translateX(0)',
          transition:`transform 1s ${easing} 0.1s`,
        }}>
          {/* Blueprint image inside left door */}
          <div style={{
            position:'absolute', top:0, bottom:0, left:0, width:'200%',
            backgroundImage:`url('/blueprint.jpg')`, backgroundSize:'cover', backgroundPosition:'left center',
            transform: assembled ? 'translateX(0) scale(1)' : 'translateX(-6%) scale(1.08)',
            transition:'transform 1.4s ease',
          }} />
          <div style={{ position:'absolute', inset:0, background:'linear-gradient(180deg,rgba(6,13,27,.2),rgba(6,13,27,.05) 40%,rgba(6,13,27,.6))' }} />
          {/* Gold edge */}
          <div style={{ position:'absolute', top:0, right:0, bottom:0, width:2, background:`linear-gradient(180deg,transparent,${G},transparent)`, boxShadow:`0 0 20px ${G}` }} />
        </div>

        {/* RIGHT DOOR */}
        <div style={{
          position:'absolute', top:0, right:0, bottom:0, width:'50%', zIndex:10, overflow:'hidden',
          transform: doorsOpen ? 'translateX(101%)' : 'translateX(0)',
          transition:`transform 1s ${easing} 0.1s`,
        }}>
          <div style={{
            position:'absolute', top:0, bottom:0, right:0, width:'200%',
            backgroundImage:`url('/blueprint.jpg')`, backgroundSize:'cover', backgroundPosition:'right center',
            transform: assembled ? 'translateX(0) scale(1)' : 'translateX(6%) scale(1.08)',
            transition:'transform 1.4s ease',
          }} />
          <div style={{ position:'absolute', inset:0, background:'linear-gradient(180deg,rgba(6,13,27,.2),rgba(6,13,27,.05) 40%,rgba(6,13,27,.6))' }} />
          {/* Gold edge */}
          <div style={{ position:'absolute', top:0, left:0, bottom:0, width:2, background:`linear-gradient(180deg,transparent,${G},transparent)`, boxShadow:`0 0 20px ${G}` }} />
        </div>

        {/* HERO TEXT — visible when doors closed */}
        <div style={{
          position:'absolute', inset:0, zIndex:11, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', textAlign:'center', padding:'0 24px',
          opacity: heroText ? 1 : 0,
          transform: heroText ? 'none' : 'translateY(-24px)',
          transition:'opacity .5s ease, transform .5s ease',
          pointerEvents: heroText ? 'auto' : 'none',
        }}>
          <p style={{ fontSize:11, letterSpacing:'.32em', textTransform:'uppercase', color:G, marginBottom:22, opacity: assembled ? 1 : 0, transition:'opacity .8s .6s' }}>Off-Plan Specialist &nbsp;·&nbsp; Dubai</p>
          <h1 style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:'clamp(52px,9vw,106px)', fontWeight:300, lineHeight:1.02, marginBottom:28 }}>
            ...
            <br/>
            <em style={{color:G}}>
              {['Investment','Analyst'].map((w,i)=><span key={i} className="hero-word" style={{animationDelay:`${.74+i*.12}s`,marginRight:'.22em'}}>{w}</span>)}
            </em>
            <br/>
            {['in','Dubai'].map((w,i)=><span key={i} className="hero-word" style={{animationDelay:`${1+i*.12}s`,marginRight:'.22em'}}>{w}</span>)}
          </h1>
          <p style={{ fontSize:15, color:M, maxWidth:520, margin:'0 auto 44px', fontWeight:300, opacity: assembled ? 1 : 0, transition:'opacity .8s 1s' }}>
        ...
        {/* SEARCH BAR — rises from center when doors open */}
        <div style={{
          position:'absolute', inset:0, zIndex:12, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'0 48px',
          opacity: doorsOpen ? 1 : 0,
          transform: doorsOpen ? 'translateY(0)' : 'translateY(50px)',
          transition:'opacity .7s ease .7s, transform .7s cubic-bezier(0.34,1.2,0.64,1) .7s',
          pointerEvents: doorsOpen ? 'auto' : 'none',
        }}>
          {/* Gold lines radiating out */}
          <div style={{ position:'absolute', top:'50%', left:0, right:0, height:1, background:`linear-gradient(90deg,transparent,rgba(201,168,76,.2),rgba(201,168,76,.5),rgba(201,168,76,.2),transparent)`, transform:'translateY(-1px)', pointerEvents:'none' }} />

          <p style={{ fontSize:10, letterSpacing:'.35em', textTransform:'uppercase', color:T, marginBottom:16 }}>Analyst Intelligence</p>
          <h2 style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:'clamp(26px,4vw,52px)', fontWeight:300, lineHeight:1.08, marginBottom:36, textAlign:'center' }}>
            Analyse Any Dubai Project<br/><em style={{color:G}}>the Way Institutions Do</em>
          </h2>

          {/* Search bar */}
          <div className="hero-search-wrap" style={{ display:'flex', width:'100%', maxWidth:760, border:`1px solid rgba(201,168,76,.35)`, overflow:'hidden', background:'rgba(6,13,27,.85)', backdropFilter:'blur(24px)', boxShadow:`0 0 60px rgba(0,0,0,.6), 0 0 30px rgba(201,168,76,.08)`, animation:'glow 3s ease-in-out infinite' }}>
            <input
            ...
            />
            <button className="hero-gen-btn" onClick={()=>generate()} disabled={loading}>
              {loading ? 'Analysing…' : 'Generate Brief'}
            </button>
          </div>
          <p style={{ marginTop:18, fontSize:12, color:'rgba(240,234,214,.35)', letterSpacing:'.12em', textTransform:'uppercase' }}>
            e.g. Sobha Siniya Island &nbsp;·&nbsp; Ramada Residences &nbsp;·&nbsp; Binghatti Aurora
          </p>
        </div>
      </section>
      ...
      {/* ═══════════════════════ */}
      <section id="brief-section" style={{ padding:'100px 48px', position:'relative' }}>
        <div style={{ maxWidth:860, margin:'0 auto' }}>
          {/* If no brief yet, show prompt */}
          {!loading && !brief && (
            <div style={{ textAlign:'center' }}>
              ...
              <Reveal delay={.1}><h2 style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:'clamp(32px,4vw,58px)', fontWeight:300, lineHeight:1.08, marginBottom:20 }}>Get Your Investment Brief<br/><em style={{color:G}}>Scroll Up to Enter a Project</em></h2></Reveal>
            </div>
          )}
                    ...
                    <div className="gate-form" style={{ display:'flex', gap:10, width:'100%', maxWidth:500, flexWrap:'wrap', justifyContent:'center' }}>
                      {[{v:name,s:setName,p:'Your name'},{v:phone,s:setPhone,p:'WhatsApp (+91 or +971…)'}].map((f,i)=>(
                        <input key={i} value={f.v} onChange={e=>f.s(e.target.value)} placeholder={f.p}
                          style={{ flex:1, minWidth:160, padding:'13px 18px', background:'rgba(255,255,255,.04)', border:gb, color:C, fontSize:13 }} />
                      ))}
          ...
          <Reveal>
            <div style={{ position:'relative' }}>
              ...
              <div style={{ width:'100%', aspectRatio:'3/4', background:`linear-gradient(135deg,${N2},rgba(20,184,166,.06))`, border:gb, display:'flex', alignItems:'center', justifyContent:'center' }}>
                <span style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:90, fontWeight:300, color:'rgba(201,168,76,.12)', letterSpacing:'-.02em' }}>N.</span>
              </div>
