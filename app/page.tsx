...
const WHY = [
  ...
  { icon: '🏠', title: 'Golden Visa', body: 'A AED 2M+ investment (approx. USD 545K / GBP 430K / INR 4.6 Cr) unlocks a 10-year UAE residency visa.' },
...
const LOAD_STEPS = [
  ...
  'Computing yield model…',
  'Running 3-year scenarios…',
...
export default function Home() {
  ...
  const [waPhone, setWaPhone] = useState('')   // pre-filled from CTWA URL param
  ...
  function submitLead() {
    ...
    fetch('/api/brief', {
      ...
      body: JSON.stringify({
        ...
        clientName,
        clientPhone: waPhone ? `+${waPhone}` : fullPhone,
    ...
    <main style={{ background: N, color: C, fontFamily: 'Inter,sans-serif', minHeight: '100vh', overflowX: 'hidden' }}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        input,button,select{font-family:Inter,sans-serif;outline:none}
        ...
        .entry-tab{padding:12px 20px;background:transparent;border:1px solid rgba(255,255,255,.08);color:#8A7F6E;font-size:11px;letter-spacing:.08em;text-transform:uppercase;cursor:pointer;transition:all .2s}
      ...
      {cookieConsent === null && (
        <div style={{ position:'fixed', bottom:0, left:0, right:0, zIndex:999, background:'rgba(12,22,40,.97)', backdropFilter:'blur(16px)', borderTop:`1px solid rgba(201,168,76,.2)`, padding:'20px 48px', display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:16 }}>
          ...
          <div style={{ display:'flex', gap:10 }}>
            <button onClick={rejectCookies} style={{ padding:'9px 20px', background:'transparent', border:'1px solid rgba(255,255,255,.12)', color:M, fontSize:11, letterSpacing:'.08em', textTransform:'uppercase', cursor:'pointer' }}>Reject</button>
            <button onClick={acceptCookies} style={{ padding:'9px 20px', background:G, border:'none', color:N, fontSize:11, letterSpacing:'.08em', textTransform:'uppercase', fontWeight:600, cursor:'pointer' }}>Accept</button>
          </div>
        </div>
      )}
      ...
      <nav style={{ position:'fixed', top:0, left:0, right:0, zIndex:100, display:'flex', alignItems:'center', justifyContent:'space-between', padding:scrolled?'14px 48px':'22px 48px', background:scrolled?'rgba(6,13,27,.95)':'rgba(6,13,27,.8)', backdropFilter:'blur(20px)', borderBottom:gb, transition:'padding .3s,background .3s' }}>
        <img src="/nawazsellsdubai_logo_nobg.png" alt="Nawaz Sells Dubai" style={{ height:72, width:'auto' }} />
        ...
        <a href="https://wa.me/971563281781" target="_blank"
          style={{ padding:'9px 22px', border:`1px solid ${G}`, color:G, fontSize:11, letterSpacing:'.12em', textTransform:'uppercase', textDecoration:'none', transition:'all .2s' }}
            ...
            <div style={{ animation:'fadeIn .4s ease' }}>
                ...
                <div style={{ animation:'fadeIn .3s ease', background:'rgba(255,255,255,.04)', border:`1px solid rgba(201,168,76,.25)`, padding:28, boxShadow:'0 8px 40px rgba(0,0,0,.3)' }}>
                  <label style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:12, padding:'36px 20px', border:'1px dashed rgba(201,168,76,.3)', cursor:'pointer', transition:'border-color .2s' }}
                    ...
                    <div style={{ textAlign:'center' }}>
                      <div style={{ fontSize:13, color:C, marginBottom:4 }}>Drop your sales offer or brochure here</div>
                  ...
                  {uploadedFile && (
                    <div style={{ marginTop:16, padding:'12px 16px', background:'rgba(20,184,166,.08)', border:'1px solid rgba(20,184,166,.2)', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
                      ...
                      <div style={{ display:'flex', gap:12, alignItems:'center' }}>
                        ...
                        <button onClick={() => { setUploadedFile(''); setUploadedBase64(''); setUploadedMime('') }} style={{ background:'none', border:'none', color:M, cursor:'pointer', fontSize:18 }}>×</button>
                      </div>
                    </div>
                  )}
                </div>
              ...
              {entryMode === 'explore' && (
                <div style={{ animation:'fadeIn .3s ease', background:'rgba(255,255,255,.04)', border:`1px solid rgba(201,168,76,.25)`, padding:28, boxShadow:'0 8px 40px rgba(0,0,0,.3)' }}>
                  ...
                  <button className="gen-btn" style={{ width:'100%' }} onClick={startSurvey}>Let's find your match →</button>
                </div>
              )}
          ...
          {stage === 'survey' && (
            <div style={{ animation:'fadeIn .4s ease', maxWidth:600, margin:'0 auto' }}>
              <div style={{ display:'flex', gap:8, marginBottom:48 }}>
                {SURVEY.map((_,i) => <div key={i} style={{ flex:1, height:2, background:i<=surveyStep?G:'rgba(255,255,255,.1)', transition:'background .3s' }} />)}
              </div>
              ...
              <h2 style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:'clamp(24px,3.5vw,42px)', fontWeight:300, lineHeight:1.15, marginBottom:36 }}>{SURVEY[surveyStep].q}</h2>
              ...
              {gated ? (
                <div>
                  ...
                  <div style={{ background:'rgba(12,22,40,.95)', border:`1px solid rgba(201,168,76,.25)`, padding:36, boxShadow:'0 20px 60px rgba(0,0,0,.5)' }}>
                    ...
                    <p style={{ fontSize:10, color:'rgba(138,127,110,.5)', marginTop:14, lineHeight:1.6 }}>
                      By submitting, you agree to our <a href="/privacy" style={{ color:T, textDecoration:'none' }}>Privacy Policy</a>. Your data will only be used to deliver this brief and for follow-up.
                    </p>
                  </div>
                </div>
              ) : (
      ...
      <footer style={{ padding:'40px 48px', borderTop:gb }}>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:20, marginBottom:24 }}>
          <img src="/nawazsellsdubai_logo_nobg.png" alt="Nawaz Sells Dubai" style={{ height:88, width:'auto' }} />
